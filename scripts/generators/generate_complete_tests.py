...
"""
    ...
    def _generate_page_object_content(self, name: str, discovery_results: Dict[str, Any]) -> str:
        ...
        for element in elements:
            ...
            if category == "input":
                methods.append(f"""
    async def fill_{element_name}(self, value):
        \"\"\"
        ...
        Args:
            value: Value to fill
        \"\"\"
        await self.fill(self.{element_name}_selector, value)""")
    ...
    async def click_{element_name}(self):
        ...
        \"\"\"
        await self.click(self.{element_name}_selector)""")
            elif category == "link":
                methods.append(f"""
    async def click_{element_name}_link(self):
        \"\"\"
        ...
        await self.click(self.{element_name}_selector)""")
                ...
                methods.append(f"""
    async def select_{element_name}(self, value):
        \"\"\"
        ...
        Args:
            value: Value to select
        \"\"\"
        await self.select(self.{element_name}_selector, value)""")
...
{chr(10).join(selectors)}
    ...
    async def navigate(self):
        ...
        Navigate to {name}
        \"\"\"
        await super().navigate("{discovery_results.get('url')}")
{"".join(methods)}
"""
...
class TestNavigation:
    ...
    def test_navigation(self):
        ...
        # Verify page title
        assert "{discovery_results.get('title')}" in self.page.title(), "Page title does not match"
"""
