#!/bin/bash
# Run the fixed proper multi-agent workflow
...
# Run the workflow
echo "Running fixed proper multi-agent workflow for $NAME at $URL..."
if [ "$HEADLESS" = true ]; then
  python fixed_proper_multi_agent_workflow.py --url "$URL" --name "$NAME" --headless
else
  python fixed_proper_multi_agent_workflow.py --url "$URL" --name "$NAME" --no-headless
fi
